...
user_balances = {}
last_checkin = {} # Quản lý ngày điểm danh của từng người (1 lần/ngày)
running_games = {} # Quản lý trạng thái tự động theo kênh (channel_id)

# MÃ CODE ADMIN: Gõ !admin pro để nhận 50k xu bất cứ lúc nào
...
def get_balance(user_id):
    if user_id not in user_balances:
        user_balances[user_id] = 100000 # Mặc định khởi điểm 100k xu
...
# ==========================================
# LỆNH XEM SỐ DƯ, ĐIỂM DANH & MÃ ADMIN
# ==========================================
...
async def admin_code(ctx, code: str = ""):
    """Lệnh dành riêng cho admin nhận xu với mã pro. Cú pháp: !admin pro"""
    if code == SECRET_ADMIN_CODE:
        ...
        try:
            ...
            pass
        await ctx.send(f"👑 **[ADMIN PRO]** Chào sếp **{ctx.author.display_name}**! Đã nạp thành công **+50,000 xu** vào tài khoản qua mã `pro`. Số dư mới: **{user_balances[uid]:,} xu** 🔥", delete_after=10)
    else:
        try:
            await ctx.message.delete()
        except:
            pass
        await ctx.send(f"❌ **{ctx.author.display_name}**: Mã code Admin không chính xác!", delete_after=5)
...
# ==========================================
# VÒNG LẶP TÀI XỈU TỰ ĐỘNG CHUẨN XÁC XUẤT THẬT
# ==========================================
...
async def taixiu(ctx):
    ...
    await ctx.send("🎰 **[SÒNG CASINO HUY UY TÍN]** Bắt đầu chuỗi ván chơi tự động liên tục chuẩn xác suất thật! Gõ `!stop` để dừng lại bất cứ lúc nào.")
    ...
    while running_games.get(channel_id):
        ...
        # THUẬT TOÁN XÁC SUẤT XÚC XẮC THẬT (Độc lập 100% qua hệ thống phân phối chuẩn)
            ...
            for uid, user_bets in view.bets.items():
                for b in user_bets:
                    ...
                    if choice == kq_taixiu or choice == kq_chanle:
                        ...
                        is_win = True
                        amount *= 5 # Trúng số chính xác ăn x5
        ...
        # Đếm ngược 5 giây sang ván tiếp theo
