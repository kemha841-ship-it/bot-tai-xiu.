...
@bot.event
async def on_ready():
    # Đồng bộ Slash Commands để bot hiện lệnh /admin
    try:
        await bot.tree.sync()
...
# ==========================================
# LỆNH XEM SỐ DƯ, ĐIỂM DANH & SLASH ADMIN (ẨN MÌNH THẤY)
# ==========================================
...
# LỆNH ADMIN DẠNG SLASH (/admin pro) - CHỈ MỘT MÌNH ÔNG THẤY KẾT QUẢ (EPHEMERAL)
...
async def slash_admin(interaction: discord.Interaction, code: str):
    if code == SECRET_ADMIN_CODE:
        ...
        user_balances[uid] = get_balance(uid) + 50000
        # ephemeral=True giúp chỉ một mình ông nhìn thấy tin nhắn này, người khác hoàn toàn không thấy gì
