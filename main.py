...
user_balances = {}
last_checkin = {} # Quản lý điểm danh 1 lần/ngày
running_games = {} # Quản lý trạng thái tự động theo kênh
recent_results = [] # Lưu lịch sử kết quả các ván gần nhất để soi cầu
...
# ==========================================
# CÁC LỆNH TIỆN ÍCH & SLASH COMMANDS MỚI
# ==========================================
...
# 2. Lệnh xem bảng xếp hạng đại gia (/top)
...
# 3. Lệnh xem lịch sử soi cầu gần nhất (/lichsu)
...
async def slash_lichsu(interaction: discord.Interaction):
    if not recent_results:
        await interaction.response.send_message("📊 Chưa có ván chơi nào diễn ra trong phiên này!", ephemeral=True)
        return
        
    history_str = "\n".join(recent_results[-10:])
    embed = discord.Embed(
...
# 4. Lệnh chuyển xu bằng cách tag @ thành viên (/chuyenxu)
...
async def taixiu(ctx):
    ...
    while running_games.get(channel_id):
        ...
        recent_results.append(f"🎲 `{DICE_EMOJIS[d1]} {DICE_EMOJIS[d2]} {DICE_EMOJIS[d3]}` = **{tong}** ➔ **{kq_taixiu.upper()}** ({kq_chanle.upper()})")
